...
void smvar_conv_code_nCi(
	...
	if(bias){
		ddr_addr = bias_ddr_addr;
		ddr_step = 8;
		ram_addr = 0;
		ram_step = 0;
		bloc_size = Co/32;
		bloc_numb = 1+1*32;
		smvar_bias_dma_isa_gen(
			ddr,
			biasSram,
			ddr_addr,
			ddr_step,
	...
	int   mat_ddr_addr2=mat_ddr_addr;
    ...
    bloc_size=0;
	for(int k=0; k<K*K; k++){
		bloc_size = bloc_size + non_numb[k]/32;
	}
//    bloc_size = K*K*Co;
	bloc_numb = 1+1*32;
//    bloc_numb = 1+1*16;
    ...
    vec_ddr_addr = fea_ddr_addr-Wi*P*Ci-P*Ci;
	...
	for(int h = 0; h < bc_h; h=h+1){
		for(int w = 0; w < bc_w; w++){
            for(int ci = 0; ci < bc_ci; ci++){
            	...
            	//***********modified for spmat
            	if(ci==(bc_ci-1)){
            		mat_ddr_addr2=mat_ddr_addr;
            	}
            	else{
        			for(int k=0; k<K*K; k++){
        				mat_ddr_addr2 = mat_ddr_addr2+non_numb[ci*K*K+k];
        			}
            	}
				...
				if(ci == bc_ci-1){
					...
					for(int k=0; k<K*K; k++){
						bloc_size = bloc_size + non_numb[(ci+1)*K*K+k]/32;
					}
				}
                //bloc_size = Co*K*K;
				bloc_numb = 1+1*32;
...
//                cout<<"check 2"<<endl;
				...
				if(K>1)
					subconv_12x12x32(
						...
						SumSram, ResVSram, ResMSram,vecReg,matReg,res_bus,sum,virtual_mat_block,&non_numb[ci*K*K],
...
//			    if(h==0 && w==0 &&ci==1){
//			    	cout<<" block index h:"<<h<<" w:"<<w<<" ci:"<<ci<<endl;
//			    }
//			    if(K>1)
//			    	if(h==0 && w==0){
