...
//	cout<<" upsample"<<endl;
    unsigned int vec_ddr_addr = fea_ddr_addr1;
    unsigned int res_ddr_addr_t = res_ddr_addr;
	...
	//diff in ddr_addr
	ddr_addr = vec_ddr_addr+2147483648;
	ddr_step = Ci*16/64;
	ram_addr = 0+P;
	...
	for(int h = 0; h < bc_h; h=h+1){
		for(int w = 0; w < bc_w; w++){
            for(int ci = 0; ci < bc_ci; ci++){
			    ...
			    subupsample_12x12x32(ddr, VecSram0, VecSram1, VecRegs0, VecRegs1,ResVSram, ResMSram,
						vec_base_addr, mat_base_addr, res_dest_addr, K,P, Co, S, isa_idx,isa_ddr);


				ddr_addr = res_ddr_addr_t;
				ddr_step = 8*Co/32;
				//diff because fuse maxpool and cat
				//ddr_step = 8*Co/32*4;
				ram_addr = 0;
				//ram_step may overflow??
				...
				bloc_size = 2;
//		        if(bc_h == 1)
//		        	bloc_numb = Hi/S+Wi/S*32;
//		        else if(bc_h > 1){
//		        	if(h < bc_h - 1)
//		        		bloc_numb = 12+Wi/S*32;
//		        	else
//		        		bloc_numb = Hi/S - 12*(bc_h-1)+Wi/S*32;
//		        }
		        if(bc_h == 1)
		        	bloc_numb = Hi*2/S+Wi*Co/32/S*32;
		        else if(bc_h > 1){
		        	if(h < bc_h - 1)
		        		bloc_numb = 12*2+Wi*Co/32/S*32;
		        	else
		        		bloc_numb = Hi*2/S - 12*2*(bc_h-1)+Wi*Co/32/S*32;
		        }
		        //diff compare 2 maxpool
//		        bloc_size = 1;
...
//				cout<<"add_layer smvar_res_dma_isa_gen = "<<endl;
			    ...
			    if(w < bc_w - 1 && ci < bc_ci - 1)
			    	...
			    	res_ddr_addr_t = res_ddr_addr_t + 12*Co*2 - Co + 32;
			    else if(w == bc_w - 1 && ci < bc_ci - 1)
			    	res_ddr_addr_t = res_ddr_addr_t + 32;
			    else
			    	res_ddr_addr_t = res_ddr_addr_t + 12*Co*2*Wi*2/S - (bc_w-1)*12*2*Co - Co + 32;

//		        if(w < bc_w - 1 && ci < bc_ci - 1)
//		        	res_ddr_addr_t = res_ddr_addr_t + 2*32;
//				else if(w < bc_w - 1 && ci == bc_ci - 1)
//					res_ddr_addr_t = res_ddr_addr_t + 12*Co*2*2 - Co*2 + 2*32;
//				else if(w == bc_w - 1 && ci < bc_ci - 1)
//					res_ddr_addr_t = res_ddr_addr_t + 2*32;
//				else
//					res_ddr_addr_t = res_ddr_addr_t + 12*Co*2*2*Wi*2/S - (bc_w-1)*12*2*Co*2 - Co*2 + 2*32;
            }
		}
	}
}
