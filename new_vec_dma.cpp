...
//                cout<<"check 2"<<endl;
                ...
                if(bc_h == 1){
                    if(bc_w == 1){
                        if(ci < bc_ci - 1){
                            vec_ddr_addr = vec_ddr_addr+32/32;
			    	        ram_step = (12*S+2*P)*32/32+4096*15;//bloc_w = 64;
			    	        bloc_size = Wi-(bc_w-1)*(S*12)+2*P;
			    	        bloc_numb = Hi-(bc_h-1)*(hbsize*S)+2*P+Wi*32;
                        }
                        else {
                            vec_ddr_addr = vec_ddr_addr+12*S*Ci/32-(bc_ci-1)*32/32;
                    ...
                    if(bc_w == 1){
                        if(h==0){
                            if(ci < bc_ci - 1){
			    	            ...
			    	            bloc_size = Wi-(bc_w-1)*(S*12)+2*P;
			    	            bloc_numb = (hbsize*S+2*P)+Wi*32;
                            }
                            else {
                                vec_ddr_addr = vec_ddr_addr+(hbsize*S)*Wi*Ci/32-(bc_w-1)*(12*S)*Ci/32-(bc_ci-1)*32/32;
